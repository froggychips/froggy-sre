...
public actor MCPServer {
    ...
    private func analyzeTool(args: [String: Any]) async -> [String: Any] {
        ...
        do {
            ...
            try? await store.save(report) // best-effort — don't fail analysis on write error
    ...
    private func historyTool(args: [String: Any]) async -> [String: Any] {
        ...
        do {
            ...
            guard !incidents.isEmpty else {
                return textContent("No incidents recorded yet.")
            }
            let lines = incidents.enumerated().map { i, s in formatStored(index: i + 1, stored: s) }
    ...
    private func toolList() -> [[String: Any]] {
        [
            [
                "name": "sre_analyze",
                "description": "Analyze a Kubernetes incident through the 5-stage pipeline: Analyzer → Hypothesis → Critic → Fix → Risk. Result is saved to incident history.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
    ...
    private func formatStored(index: Int, stored: StoredIncident) -> String {
        ...
        return """
        \(index). \(ts) — \(alert)\(namespace) — Risk: \(score)
